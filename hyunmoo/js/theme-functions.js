...
function searchProducts(response) {
	...
	if(products.length == 0){
		...
		$.each(products,function(key,value){
			...
			$(".search-content #product"+i).find(".proddetails").append("<span class='searchprice'></span>");
			$(".search-content #product"+i).find(".proddetails").append("<div class='clear'></div>");
...
function searchPosts(response) {
	...
	if(posts.length==0){
		...
		$.each(posts,function(key,value){
			...
			if(value['image'])
				$(".search-content #post"+i+" .postcontent").append("<div class='postthumb'>"+value['image']+"</div>");
		...
		$(".post .postthumb div.hmslider-post").each(function(){
			$(this).owlCarousel({
...
function searchPages(response) {
	...
	if(pages.length==0){
		...
		$.each(pages,function(key,value){
			...
			if(value['image'])
				$(".search-content #page"+i+" .postcontent").append("<div class='postthumb'>"+value['image']+"</div>");
...
function loadProducts(response) {
	...
	$.each(products, function(key,value) {
		...
		$("ul.products li#product"+i+" a div.prodimagecontainer").append("<div class='priceandnamecontainer'>");
		var prodname=value['title'];
		if(prodname.length>10)
			prodname=prodname.substring(0,10)+"...";
		$("ul.products li#product"+i+" a div.priceandnamecontainer").append("<h3>"+prodname+"</h3>");
		$("ul.products li#product"+i+" a div.priceandnamecontainer").append("<span class='price'>"+value['price']+"</span>");
		if(value['rating']){
					$("ul.products li#product"+i+" a div.priceandnamecontainer").append("<div class='star-rating onload'><span style='width:"+parseFloat(value['rating'])/5*100+"%;'></span></div>"); }
					for(var kk=0;kk<5;kk++){
						$("ul.products li#product"+i+" a div.priceandnamecontainer .star-rating").append("<i class='fa fa-star-o' style='left:"+kk*16+"px;'></i>");
						$("ul.products li#product"+i+" a div.priceandnamecontainer .star-rating span").append("<i class='fa fa-star' style='left:"+kk*16+"px;'></i>");
					} 
		$("ul.products li#product"+i+" a div.prodimagecontainer").append("<div class='addtocartbutcontainer' align='center'><a class='skin-primary add_to_cart_button' rel='"+value['id']+"' href='"+value['cart']+"'>Add to cart</a></div>");

	});
	$("ul.products li a").mouseover(function(){
		$(this).find(".add_to_cart_button").css("opacity",1);
		$(this).parent().find(".priceandnamecontainer").css("opacity",1);
	});
	$("ul.products li a").mouseleave(function(){
		$(this).find(".add_to_cart_button").css("opacity",0);
		$(this).parent().find(".priceandnamecontainer").css("opacity",0);
	});
	//AJAX Add To Cart		
