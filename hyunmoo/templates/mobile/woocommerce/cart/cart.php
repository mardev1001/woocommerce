...
<table class="shop_table cart" cellspacing="0">
	...
	<tbody>
			...
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				...
				if ( $_product->exists() && $values['quantity'] > 0 ) {
					...
					<tr class = "<?php echo esc_attr( apply_filters('woocommerce_cart_table_item_class', 'cart_table_item', $values, $cart_item_key ) ); ?>">
						...
						<!-- Product price -->
						<tr><th class="product-price"><?php _e( 'Price', 'woocommerce' ); ?></th><td class="product-price">
							<?php
						...
						<!-- Quantity inputs -->
						<tr><th class="product-quantity"><?php _e( 'Quantity', 'woocommerce' ); ?></th><td class="product-quantity">
							<?php
						...
						<!-- Product subtotal -->
						<tr><th class="product-subtotal"><?php _e( 'Sub Total', 'woocommerce' ); ?></th><td class="product-subtotal">
							<?php
		...
		<?php
			echo apply_filters( 'woocommerce_cart_item_total', $woocommerce->cart->get_cart_total());
		?>
		</td></tr></tbody></table>
		<div class="couponandotherbuttons">
				<div class="heading"></div>
				<div class="updateandcheckoutbut"><input type="submit" class="skin-secondary button" name="update_cart" value="<?php _e( 'Update Cart', 'woocommerce' ); ?>" /></div><div class="updateandcheckoutbut bordertopnone"> <input type="submit" class="skin-secondary checkout-button button alt" name="proceed" value="<?php _e( 'Proceed to Checkout &rarr;', 'woocommerce' ); ?>" /></div><?php if ( $woocommerce->cart->coupons_enabled() ) { ?>
					<div class="coupon">

						<h1><label for="coupon_code"><?php _e( 'Coupon-code', 'woocommerce' ); ?></label></h1><input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="Have a coupon code?" /> <input type="submit" class="skin-secondary button" name="apply_coupon" value="<?php _e( 'Apply', 'woocommerce' ); ?>" />
