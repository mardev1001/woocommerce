...
<div id="order_review">
							...
							echo '
								<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									<th class="product-name">' .
										apply_filters( 'woocommerce_checkout_product_title','<span class="product-name1">'. $_product->get_title(), $_product ) . '</span> ' .
	...
	<div id="payment">
			...
			<?php
				...
				if ( ! empty( $available_gateways ) ) {
					...
					foreach ( $available_gateways as $gateway ) {
						?>
						<li>
							<div><input type="radio" id="payment_method_<?php echo $gateway->id; ?>" class="input-radio" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?> /><i class="fa fa-dot-circle-o radiobox"></i>
							<label for="payment_method_<?php echo $gateway->id; ?>"><?php echo $gateway->get_title(); ?> <?php echo $gateway->get_icon(); ?></label></div>
							<?php
		...
		<div class="form-row place-order">
			...
			echo apply_filters('woocommerce_order_button_html', '<input type="submit" class="skin-secondary button alt" name="woocommerce_checkout_place_order" id="place_order" value="' . $order_button_text . '" />' );
			?>
